    ...
    Papa.parse<CSVRow>('/student_lifestyle_dataset.csv', {
      ...
      complete: (results: ParseResult<CSVRow>) => {
        ...
        const scatterData: [number, number][] = formattedData.map((row) => [
          parseFloat(row.sleep), // Convert back to number for scatter plot
  ...
  const scatterPlotOption = {
    ...
    tooltip: {
      ...
      formatter: (params: { value: [number, number] }) => {
        return `Sleep Hours: ${params.value[0]}<br>GPA: ${params.value[1]}`;
      },
    },
    series: [
      {
...
export default ScatterPlotSlpTvGpa;
